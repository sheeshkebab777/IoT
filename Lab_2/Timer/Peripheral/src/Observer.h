...
{	
	...
	if(SINK_NODE == YES_SINK_NODE){
		...
		if(pack.type == FLAG_NETWORK_FORM){
			...
			if(packet.recvNodeID != pack.nodeID) return;
		...
		else if(pack.type == FLAG_NETWORK_START_SEND && packet.recvNodeID == pack.nodeID && !sending){
			sending = true;
			...
			// ! init random timer based on node number
